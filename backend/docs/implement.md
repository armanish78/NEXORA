...
4. **Install dependencies**:
   ...
   ```
   *(Note: The first time the system runs, it will automatically download the `all-MiniLM-L6-v2` embedding model and the `Qwen2.5-3B-Instruct` AI model. The system uses `bitsandbytes` for 4-bit quantization to prevent OOM errors on 6GB GPUs. Ensure you have internet access and at least 10GB of free disk space).*
